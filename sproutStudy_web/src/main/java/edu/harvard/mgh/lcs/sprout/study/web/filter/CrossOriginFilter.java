...
//	private static final Logger log = Logger.getLogger(CrossOriginFilter.class.getName());
    ...
    public void init(FilterConfig config) throws ServletException
    {
        ...
        if (allowedHeadersConfig == null)
            allowedHeadersConfig = "X-Requested-With,Content-Type,Accept,Origin,Authorization,WWW-Authenticate";
        ...
        {
//            log.info(String.format("Cross-origin filter, could not parse '%s' parameter as integer: %s", PREFLIGHT_MAX_AGE_PARAM, preflightMaxAgeConfig));
        }
    ...
    private void handle(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException
    {
        ...
        if (origin != null && isEnabled(request)) {
            if (originMatches(origin))
            {
                ...
                {
//                    log.info(String.format("Cross-origin request to %s is a simple cross-origin request", request.getRequestURI()));
                    handleSimpleResponse(request, response, origin);
                }
                else if (isPreflightRequest(request))
                {
//                    log.info(String.format("Cross-origin request to %s is a preflight cross-origin request", request.getRequestURI()));
                    handlePreflightResponse(request, response, origin);
//                    if (chainPreflight)
//                        log.debug(String.format("Preflight cross-origin request to %s forwarded to application", request.getRequestURI()));
//                    else
                        return;
                }
                else
                {
//                    log.info(String.format("Cross-origin request to %s is a non-simple cross-origin request", request.getRequestURI()));
                    handleSimpleResponse(request, response, origin);
                }
            }
            else
            {
//                log.info("Cross-origin request to " + request.getRequestURI() + " with origin " + origin + " does not match allowed origins " + allowedOrigins);
            }
        }
...
//        log.debug(String.format("%s is %s", ACCESS_CONTROL_REQUEST_METHOD_HEADER, accessControlRequestMethod));
        boolean result = false;
        if (accessControlRequestMethod != null)
            result = allowedMethods.contains(accessControlRequestMethod);
//        log.debug(String.format("Method %s is" + (result ? "" : " not") + " among allowed methods %s", accessControlRequestMethod, allowedMethods));
        return result;
    }
...
//        log.debug(String.format("%s is %s", ACCESS_CONTROL_REQUEST_HEADERS_HEADER, accessControlRequestHeaders));
        boolean result = true;
...
//        log.debug(String.format("Headers [%s] are" + (result ? "" : " not") + " among allowed headers %s", accessControlRequestHeaders, allowedHeaders));
        return result;
    }
